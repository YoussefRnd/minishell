...
/*   Updated: 2024/09/15 20:18:12 by yboumlak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	is_env_token_attached(t_token **tokens)
{
	return (((*tokens)->type == TOKEN_ENV && (*tokens)->next
			...
			&& (*tokens)->next->type != TOKEN_EOF) && (*tokens)->next->type != TOKEN_EMPTY || ((*tokens)->next
			&& (*tokens)->next->type == TOKEN_ENV) || ((*tokens)->next->type == TOKEN_EMPTY && (*tokens)->next->next->type == TOKEN_WORD));
}
